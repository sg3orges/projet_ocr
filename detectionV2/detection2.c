...
/* === Détection automatique des zones grille et mots === */
typedef struct { int start,end; double avg; } Block;
                       ...
                       int *wx0,int *wx1,int *wy0,int *wy1)
{
    ...
    /* 1️⃣ Densité verticale moyenne de noir */
    for(int x=0;x<W;x++){
        int black=0;
        for(int y=0;y<H;y++)
            if(gray(pix,x,y)<thr) black++;
        dens[x]=(double)black/H;
    }

    /* 2️⃣ Détection des bandes sombres (zones de texte) */
    ...
    /* 4️⃣ Trie par largeur décroissante */
    ...
    /* 5️⃣ Choix grille/mots selon largeur et densité */
    int w1=b1.end-b1.start, w2=b2.end-b2.start;
    bool left_is_words=false;
    if(w1<w2 || b1.avg<b2.avg*0.8) left_is_words=true;

    if(left_is_words){
        *wx0=b1.start; *wx1=b1.end; *wy0=top; *wy1=bottom;
        *gx0=b2.start; *gx1=b2.end; *gy0=top; *gy1=bottom;
    }else{
        *gx0=b1.start; *gx1=b1.end; *gy0=top; *gy1=bottom;
        *wx0=b2.start; *wx1=b2.end; *wy0=top; *wy1=bottom;
    }
...
static void run_detection(GtkWidget *win,const char *path){
    ...
    draw_rect(disp,gx0,gy0,gx1,gy1,255,0,0); // rouge : grille
    draw_rect(disp,wx0,wy0,wx1,wy1,0,255,0); // vert : mots
