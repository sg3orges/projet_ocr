...
static void on_image_clicked(GtkButton *button, gpointer user_data)
{
    ...
    // Apply the same blue background with CSS
    ...
    // Scrolling area that will contain the grid of images + buttons
    GtkWidget *scroll = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    gtk_widget_set_hexpand(scroll, TRUE);
    gtk_widget_set_vexpand(scroll, TRUE);
    gtk_widget_set_size_request(scroll, 1000, 600); // larger scrolling area
    ...
    for (size_t i = 0; i < img_count; i++) {
        ...
        if (pixbuf) {
            ...
            img_widget = gtk_label_new("Unable to load image");
        ...
        GtkWidget *btn = gtk_button_new_with_label("Choose");
...
void run_interface(int argc, char *argv[])
{
    ...
    GtkWidget *title = gtk_label_new("OCR Project");
    ...
    // If a "Choose" button was clicked, launch the rotation interface with the selected image
